...
def _blur2d(x, f=[1,2,1], normalize=True, flip=False, stride=1):
    ...
    if f.ndim == 1:
        f = f[:, np.newaxis] * f[np.newaxis, :] #f = [x, x]
    assert f.ndim == 2
    if normalize:
        f /= np.sum(f)
    if flip:
...
def _downscale2d(x, factor=2, gain=1):
    #basically downscaling performed in blurring or avgpooling
    assert x.shape.ndims == 4 and all(dim.value is not None for dim in x.shape[1:])
    assert isinstance(factor, int) and factor >= 1
    ...
    #x = tf.transpose(x, [0, 2, 3, 1]) #for CPU compatibility
    #x = tf.nn.avg_pool(x, ksize=ksize, strides=ksize, padding='VALID')#, data_format='NCHW')
    #return tf.transpose(x, [0, 3, 1, 2]) #for CPU compatibility
...
def layer_instance_norm(x, epsilon=1e-8):
    ...
    with tf.variable_scope('LayerInstanceNorm'):
        ...
        epsilon = tf.constant(epsilon, dtype=y.dtype, name='epsilon')
        ...
        epsilon = tf.constant(epsilon, dtype=z.dtype, name='epsilon')
